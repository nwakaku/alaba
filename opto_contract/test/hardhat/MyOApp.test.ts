...
describe('MyOApp Test', function () {
    ...
    // Test case to verify native drop functionality
    it('should quote fees for native drop correctly', async function () {
        const message = 'Hello with native drop!'
        const nativeDropAmount = ethers.utils.parseEther('0.1')
        const receiverAddress = ownerB.address
        
        const fee = await myOAppA.quoteSendStringWithNativeDrop(
            eidB,
            message,
            nativeDropAmount,
            receiverAddress,
        ...
        expect(fee.nativeFee.gt(0)).to.be.true
        expect(fee.lzTokenFee.eq(0)).to.be.true
    })

    // Test case to verify sending message with native drop
    it('should send a string message with native drop', async function () {
        // Assert initial state
        expect(await myOAppA.lastMessage()).to.equal('')
        expect(await myOAppB.lastMessage()).to.equal('')

        const message = 'Test message with native drop.'
        const nativeDropAmount = ethers.utils.parseEther('0.05')
        const receiverAddress = ownerB.address

        // Quote the fee for native drop
        const messagingFee = await myOAppA.quoteSendStringWithNativeDrop(
            eidB,
            message,
            nativeDropAmount,
            receiverAddress,
        ...
        // Execute sendStringWithNativeDrop operation
        await myOAppA.sendStringWithNativeDrop(
            eidB,
            message,
            nativeDropAmount,
            receiverAddress,
            { value: messagingFee.nativeFee.toString() }
        )

        // Assert the resulting state
        expect(await myOAppA.lastMessage()).to.equal('')
        expect(await myOAppB.lastMessage()).to.equal('Test message with native drop.')
    })
})
