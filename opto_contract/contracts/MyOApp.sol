...
contract MyOApp is OApp, OAppOptionsType3 {
    ...
    /**
     * @notice Quotes the gas needed for sending a string with native drop.
     * @param _dstEid Destination chain's endpoint ID.
     * @param _string The string to send.
     * @param _nativeDropAmount Amount of native gas to drop to receiver (in wei).
     * @param _receiver Address to receive the native drop.
    ...
    function quoteSendStringWithNativeDrop(
        ...
        // Create options with lzReceive gas and native drop
        bytes memory options = OptionsBuilder.newOptions()
            .addExecutorLzReceiveOption(80000, 0) // Basic gas for lzReceive
        ...
        fee = _quote(_dstEid, _message, options, _payInLzToken);
    }
    ...
    /// @notice Send a string to a remote OApp on another chain with native drop
    /// @param _dstEid   Destination Endpoint ID (uint32)
    /// @param _string  The string to send
    /// @param _nativeDropAmount Amount of native gas to drop to receiver (in wei)
    /// @param _receiver Address to receive the native drop (will be converted to bytes32)
